...
//			ovrRating = ((2 * ratings.hgt + 2*ratings.stre + 2 * ratings.spd + 4 * ratings.jmp + 3 * ratings.endu + 4 * ratings.ins + 1 * ratings.dnk + 4*ratings.ft + 1*ratings.fg + 2 * ratings.tp + 2*ratings.blk + 1*ratings.stl + 1* ratings.drb + 1 * ratings.pss + ratings.reb) / 31);
			ovrRating = ((2 * ratings.hgt + 2*ratings.stre + 2 * ratings.spd + 4 * ratings.jmp + 6 * ratings.endu + 4 * ratings.ins + 1 * ratings.dnk + 4*ratings.ft + 1*ratings.fg + 2 * ratings.tp + 4*ratings.blk + 1*ratings.stl + 1* ratings.drb + 1 * ratings.pss + ratings.reb) / 34);
...
//				ovrRating = ((2 * ratings.hgt + 2*ratings.stre + 2 * ratings.spd + 7 * ratings.jmp + 3 * ratings.endu + 5 * ratings.ins + 1 * ratings.dnk + 5*ratings.ft + 1*ratings.fg + 2 * ratings.tp + 2*ratings.blk + 1*ratings.stl + 1* ratings.drb + 1 * ratings.pss + ratings.reb) / 36);
				...
				if (risk) {
					position = 'TOP';
					ovrRating = ((2 * ratings.hgt + 2*ratings.stre + 2 * ratings.spd + 2 * ratings.jmp + 3 * ratings.endu + 2 * ratings.ins + 5 * ratings.dnk + 2*ratings.ft + 3*ratings.fg + 2 * ratings.tp + 4*ratings.blk + 1*ratings.stl + 2* ratings.drb + 1 * ratings.pss + ratings.reb) / 34);

		        } else if (teamfight) {
					position = 'ADC';
					ovrRating = ((2 * ratings.hgt + 2*ratings.stre + 2 * ratings.spd + 3 * ratings.jmp + 3 * ratings.endu + 2 * ratings.ins + 4 * ratings.dnk + 4*ratings.ft + 1*ratings.fg + 2 * ratings.tp + 4*ratings.blk + 1*ratings.stl + 1* ratings.drb + 1 * ratings.pss + ratings.reb) / 34);

				} else {
					position = 'MID';
					ovrRating = ((2 * ratings.hgt + 2*ratings.stre + 2 * ratings.spd + 3 * ratings.jmp + 3 * ratings.endu + 2 * ratings.ins + 4 * ratings.dnk + 3*ratings.ft + 2*ratings.fg + 2 * ratings.tp + 4*ratings.blk + 1*ratings.stl + 1* ratings.drb + 1 * ratings.pss + ratings.reb) / 34);
...
//    function MMR(ratings,champions,topADC,topTOP,topMID,topJGL,topSUP) {
    function MMRcalc(ovr,skillMMR) {
				...
				if (g.gameType == 1) {
					MMR = Math.round((ovr * 10) + (skillMMR / 2)); // up to 500 + 2200 + up to 500
				} else {
					MMR = Math.round((ovr * 10) + (skillMMR / 2)); // up to 500 + 2200 + up to 500
				}
...
//            ratingKeys = ["stre", "dnk", "blk", "stl"];
            ...
            //Before Val change: ["dnk", "ft", "blk", "stl","drb","tp","ins","fg"];
...
//			p.champions = {};
			...
			for (j = 0; j <  g.numChampions; j++) {
				...
				if (p.champions[j] == undefined) {
					p.champions[j] = {skill: 10};
				}

                //Change this to make it better
                if(p.champions[j].skill > 60) {
                    p.champions[j].skill = p.ratings[r].ovr * .5 + (Math.random()*60-10);
            ...
            if (fuzzedMMR < 90) {
                p.ratings[r].rank = "Immortal 2";
            } else if(fuzzedMMR < 500) {
                p.ratings[r].rank = "Immortal 3";
            } else {
                p.ratings[r].rank = "Radiant";
            }

            p.ratings[r].fuzzedMMR = fuzzedMMR;
            p.fuzzedMMR = fuzzedMMR;
        ...
        for (i = 0; i < 2; i++) {
            if (Math.random() < 0.2) {
				...
				if  (j==12) {
					...
					rawRatings[j] = limitRating(rawRatings[j] + 20);
				}
            }
        }
...
//			fn = "test";
   ...
   function nameSwiss() {
    ...
    function userID(nationality, firstname) {
       ...
       prefixRand = random.uniform(0, userIDs.prefix.length-1);
        ...
        //BaseID
        if((nationality === "NA" || nationality == "EU") && Math.random() > 0.85) {
            baseID = firstname;
        } else {
...
//Russian.    English.    Other.    Kurdish.    Russian.    Assyrian.    Ukrainian.    Greek.
    ...
    function pos(ratings) {
        ...
        // * Game: tp = Positioning, blk = Aim, stl = Clutch, drb = Movement
           ...
           //rest mid
            ...
            //main roles for val: Duelist, Smokes, Initiator, and Sentinal 
            ...
            if (ratings.hgt >= 80 && ratings.fg >= 60 && (ratings.blk >= 75 || ratings.drb >= 70)) {
                ...
                position = "Sentinal";
            } else if(ratings.ins >= 80 && ratings.spd >= 80 && ratings.stre >= 70) {
                position = "Smokes"
            } else {
                let ran = Math.random();
    ...
    function pos2(ratings, firstPosition) {
        ...
        if (ratings.blk >= 45 && ratings.stre >= 45 && firstPosition === "Duelist") {
            ...
            position = 'IGL';
        } else if(ratings.hgt >= 55 && ratings.spd >= 45 && Math.random() > 0.85) {
            position = 'FLEX';
        } else {

            //Just a random chance for now need to fix ratings later on
            if(Math.random() > 0.75 && firstPosition !== "Duelist") {
                position = 'IGL'
            } else {
                position = 'None'
            }
    ...
    function generate(tid, age, profile, baseRating, pot, draftYear, newLeague, scoutingRank,cDefault,topADC,topMID,topJGL,topTOP,topSUP) {
        var maxHgt, maxWeight, minHgt, minWeight, nationality, country, p,i, race;
		...
		if ((g.gameType  < 2 || g.gameType == 5)  && (p.tid<=5) && (p.tid>=0) && (g.regionType != "EU"))  {
		...
		} else if((g.gameType  < 2 || g.gameType == 5)  && (p.tid<=9) && (p.tid>=5) && (g.regionType != "EU")) {
            if (Math.random() > 0.1) {
                nationality = 'BR';
        ...
        //Race for face Generation
        if(nationality == "NA" || nationality == "EU") {
        ...
        } else if (nationality == "KR" || nationality == "JP" || nationality == "TW" || nationality == "CN") {
            if( chance > 0.05) {
                race = "asian"
            } else {
                race = "white"
...
//			p.champions[i].skill =  p.ratings[0].ovr+(Math.random()*100*20-10);
            ...
            if(cDefault[i].role === p.pos) {
                p.champions[i].skill =  Math.round(p.ratings[0].ovr+(Math.random()*100-20),0);
            } else {
                p.champions[i].skill =  Math.round(p.ratings[0].ovr*0.5+(Math.random()*15-10),0);
            }
        ...
        p.ratings[0].fuzzedMMR = fuzzedMMR;
        p.fuzzedMMR = fuzzedMMR;
        ...
        p.userID = userID(nationality, p.name.split(" ")[0]);
	...
	//	console.log(p.userID);
        ...
        p.face = facesnew.generate({jersey: {id: "esports"}}, { race: race });
            ...
            if (options.season !== null) {
                ...
                for (k = 0; k < p.ratings.length; k++) {
                    ...
                    for (j = 0; j < options.ratings.length; j++) {
                        ...
                        } else if(options.ratings[j] === "fuzzedMMR") {
                            fp.ratings[kk].fuzzedMMR = p.ratings[k].fuzzedMMR;
                        } else if(options.ratings[j] === "leaderboardpos"){ 
                            fp.ratings[kk].leaderboardpos = p.ratings[k].leaderboardpos
                        } else {
