...
define(["dao", "globals", "templates", "lib/bluebird", "lib/davis", "lib/html2canvas", "lib/jquery", "lib/knockout", "util/helpers", "util/lock"], function (dao, g, templates, Promise, Davis, html2canvas, $, ko, helpers, lock) {
    ...
    function title(text) {
		...
		if (g.gameType == 0) {
			document.title = text + " - VAL GM Americas";
		} else if (g.gameType == 1) {
			document.title = text + " - VAL GM Americas";
		} else if (g.gameType == 2) {
			document.title = text + " - VAL GM LCK";
		} else if (g.gameType == 3) {
			document.title = text + " - VAL GM LPL";
		} else if (g.gameType == 4) {
			document.title = text + " - VAL GM LMS";
		} else if (g.gameType == 5) {
			document.title = text + " - VAL GM Champions";
		} else {
			document.title = text + " - VAL GM";
		}
    }
